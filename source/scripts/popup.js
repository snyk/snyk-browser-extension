      ...
      <div class="vulnerability-body rounded-bottom p-3">
        ...
        <dl class="text-xs mt-0 mb-3">
          ...
          <div class="d-flex justify-content-between border-top pt-2">
            ...
            <dd class="my-0 number">${
              vulnerability.identifiers.CVE.length > 0
          ...
          <div class="d-flex justify-content-between border-top pt-2">
            ...
            <dd class="my-0 number">${
              vulnerability.identifiers.CWE.length > 0
                ? vulnerability.identifiers.CWE.join(' ')
                : '–'
            }</dd>
          </div>
          <div class="d-flex justify-content-between border-top pt-2">
        ...
        <a target="_blank" href="${
          vulnerability.url
        }?utm_medium=Referral&utm_source=Google&utm_campaign=Chrome-Extension&utm_content=Vuln" onClick="browser.tabs.create({"url": "${
      vulnerability.url
    }?utm_medium=Referral&utm_source=Google&utm_campaign=Chrome-Extension&utm_content=Vuln"})">Learn more</a>
      ...
      if (vulnerabilitiesData.status === 401) {
        ...
        if (vulnerabilityIssues.length > 0) {
          ...
          if (critical > 0) {
            ...
            document.getElementById('totalVulnerabilitiesCritical').classList.add('critical')
          }

          if (high > 0) {
            document
              .getElementById('totalVulnerabilitiesHigh')
              .querySelector('.number').textContent = high
            document.getElementById('totalVulnerabilitiesHigh').classList.add('high')
          }

          if (medium > 0) {
            document
              .getElementById('totalVulnerabilitiesMedium')
              .querySelector('.number').textContent = medium
            document.getElementById('totalVulnerabilitiesMedium').classList.add('medium')
          }

          if (low > 0) {
            document
              .getElementById('totalVulnerabilitiesLow')
              .querySelector('.number').textContent = low
        ...
        if (licenseIssues.length > 0) {
          ...
          if (high > 0) {
            document
              .getElementById('totalLicenseIssuesHigh')
              .querySelector('.number').textContent = high
          ...
          if (medium > 0) {
            document
              .getElementById('totalLicenseIssuesMedium')
              .querySelector('.number').textContent = medium
          ...
          if (low > 0) {
            document
              .getElementById('totalLicenseIssuesLow')
              .querySelector('.number').textContent = low
